  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
         `http://localhost:8000/PublicoObjetivo/actualizarPublico/${id}`,
        {
          method: "PATCH",
          headers: {
      ...
      if (!response.ok) {
        throw new Error("Error al editar editorial");
      }

      setMensaje("publico objetivo registrado correctamente");
      setNombre("");
    } catch (error) {
      setMensaje("Ocurrió un error al editar el publico Objetivo");
    }
  };
