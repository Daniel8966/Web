  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/Editorial/Registrareditorial",
        {
          method: "POST",
          headers: {
      ...
      setMensaje("Editorial registrada correctamente");
      setNombre("");
      setDireccion("");
    } catch (error) {
      setMensaje("Ocurrió un error al registrar la editorial");
    }
  };
      ...
      <div className="min-h-screen bg-slate-700 text-white flex flex-col items-center justify-center p-6">
        <div className="bg-slate-800 p-8 rounded-2xl shadow-xl w-full max-w-md">
          ...
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            <label className="text-lg font-semibold">Nombre de la Editorial</label>
            <input
              type="text"
              value={nombre}
